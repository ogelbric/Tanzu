...
# u1 !!!!! 96 env!!!!
##
# Big2_DS is used for ESXi 
# Texas2_DS is used for HAproxy and vCenter
#
#
#
#
...
#E:\Software\VMware\v7u1stuff
# Full Path to both the Nested ESXi 7.0 VA, Extracted VCSA 7.0 ISO & HA Proxy OVAs
$NestedESXiApplianceOVA = "E:\Software\VMware\v7u1stuff\Nested_ESXi7.0u1_Appliance_Template_v1.ova"
$VCSAInstallerPath = "H:"
$HAProxyOVA = "E:\Software\VMware\v7u1stuff\vmware-haproxy-v0.1.8.ova"
...
$NestedESXiHostnameToIPs = @{
    "tanzu-esxi-1" = "192.168.4.51"
    "tanzu-esxi-2" = "192.168.4.52"
    "tanzu-esxi-3" = "192.168.4.53"
}
...
$VCSAIPAddress = "192.168.4.50"
$VCSAHostname = "tanzu-vcsa-1.lab.local" #Change to IP if you don't have valid DNS
$VCSAPrefix = "24"
...
$HAProxyManagementIPAddress = "192.168.4.2/24" # Format is IP Address/CIDR Prefix
$HAProxyHostname = "tanzu-haproxy-1.cpbu.corp"
$HAProxyPort = "5556"
$HAProxyWorkloadIPAddress = "192.168.6.2/24" # Format is IP Address/CIDR Prefix
$HAProxyWorkloadGateway = "192.168.6.1"
$HAProxyWorkloadNetworkIPRange = "192.168.6.128/25" # Format is Network CIDR Notation
$HAProxyWorkloadNetwork = "orfpriv2_192_168"
...
$VMDatacenter = "DC21"
$VMCluster = "CL21"
$VMNetwork = "orfpriv2_192_168"
#$VMDatastore = "BigFastDisks3"
$VMDatastore1 = "Big2_DS"
$VMDatastore2 = "Texas2_DS"
...
if($confirmDeployment -eq 1) {
    ...
    Write-Host -ForegroundColor White $VMDatastore1
    Write-Host -ForegroundColor White $VMDatastore2
...
#orf    $datastore = Get-Datastore -Server $viConnection -Name $VMDatastore | Select -First 1
    $datastore1 = Get-Datastore -Server $viConnection -Name $VMDatastore1 | Select -First 1
    $datastore2 = Get-Datastore -Server $viConnection -Name $VMDatastore2 | Select -First 1
...
if($deployNestedESXiVMs -eq 1) {
    $NestedESXiHostnameToIPs.GetEnumerator() | Sort-Object -Property Value | Foreach-Object {
        ...
        $vm = Import-VApp -Source $NestedESXiApplianceOVA -OvfConfiguration $ovfconfig -Name $VMName -Location $cluster -VMHost $vmhost -Datastore $datastore1 -DiskStorageFormat thin
...
if($deployHAProxy -eq 1) {
    ...
    $vm = Import-VApp -Source $HAProxyOVA -OvfConfiguration $ovfconfig -Name $HAProxyDisplayName -Location $cluster -VMHost $vmhost -Datastore $datastore2 -DiskStorageFormat thin
...
if($deployVCSA -eq 1) {
        ...
        $config.'new_vcsa'.vc.datastore = $datastore2
